...
if DISCORD_REPORT:
    ...
    bot.post_message(TEST_MESSAGE, channel_id=1427792207930458273)
else:
...
class strat1(Strategy):
    ...
    def buy_condition(self, i):
        time_cond = self.data['NY'][i-1] > 0 # in ny session 
...
if OPTIMISE:
    ...
    if DISCORD_REPORT:
        bot.post_fig(buf, "📈 Optimization Results", color=COLOR_OPTIMISE, channel_id = 1427792207930458273)
...
if MONTE_CARLO:
    ...
    if DISCORD_REPORT:
        bot.post_fig(buf, "📊 Monte Carlo Results", color=MONTE_CARLO_COLOR, channel_id = 1427792207930458273)
...
if NOISE_TEST:
    ...
    if DISCORD_REPORT:
        ...
        bot.post_fig(buf, "📢 Noise Test Results", color=NOISE_TEST_COLOR, channel_id = 1427792207930458273)


result.TEST_MODE = s.Setting.MODE_BACKTEST

bot.post_embed(result.discord_stats())
